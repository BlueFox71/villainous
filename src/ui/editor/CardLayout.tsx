...
export function CardLayoutEditor({
  ...
  const setImage = (id: string, p: Partial<CardImage>) =>
    onChange({ ...card, images: (card.images ?? []).map((i) => (i.id === id ? { ...i, ...p } : i)) })

  // Importe une image (data-URL réduite) + mesure son ratio, la pose au centre, sélectionnée.
  const addImage = async (file: File | undefined) => {
    ...
    const taken = new Set((card.images ?? []).map((i) => i.id))
    let n = 1
    while (taken.has(`img-${n}`)) n++
    const im: CardImage = { id: `img-${n}`, image, x: 50, y: 50, size: DEFAULT_CARD_IMAGE_SIZE, aspect }
    onChange({ ...card, images: [...(card.images ?? []), im] })
    setSel({ kind: 'image', id: im.id })
  }
  const removeImage = (id: string) => {
    onChange({ ...card, images: (card.images ?? []).filter((i) => i.id !== id) })
    setSel(null)
  }
  // Duplique l'image : copie identique (source/taille/ratio) décalée de quelques %, sélectionnée.
  const dupImage = (id: string) => {
    const src = card.images?.find((i) => i.id === id)
    if (!src) return
    const taken = new Set((card.images ?? []).map((i) => i.id))
    let n = 1
    while (taken.has(`img-${n}`)) n++
    const copy: CardImage = { ...src, id: `img-${n}`, x: clamp(src.x + 4, 2, 98), y: clamp(src.y + 4, 2, 98) }
    onChange({ ...card, images: [...(card.images ?? []), copy] })
    setSel({ kind: 'image', id: copy.id })
  }
    ...
    const o =
      s.kind === 'text'
        ...
        : s.kind === 'box'
          ...
          : (() => {
              // symbole OU image : même modèle géométrique (centre x/y + taille).
              const el = s.kind === 'sticker' ? card.stickers?.find((x) => x.id === s.id) : card.images?.find((x) => x.id === s.id)
              const def = s.kind === 'sticker' ? DEFAULT_STICKER_SIZE : DEFAULT_CARD_IMAGE_SIZE
  ...
  const onMove = (e: React.PointerEvent) => {
    ...
    if (d.sel.kind === 'text' || d.sel.kind === 'box') {
      ...
      // symbole OU image : même géométrie ; on route vers le bon setter.
      const isSticker = d.sel.kind === 'sticker'
      const applyEl = (p: Partial<CardSticker & CardImage>) =>
        isSticker ? setSticker((d.sel as { id: string }).id, p) : setImage((d.sel as { id: string }).id, p)
      if (d.mode === 'move') {
        applyEl({ x: clamp(d.ox + dxPctW, 2, 98), y: clamp(d.oy + dyPctH, 2, 98) })
      } else {
        const delta = ((dxPctW + dyPctH) / 2) * 2
        applyEl({ size: clamp(d.osize + delta, 4, isSticker ? 60 : 120) })
      }
    }
  }
      ...
      if (sel.kind === 'text') {
        ...
        onChange({
          ...
          images: (card.images ?? []).map((i) =>
            i.id === sel.id ? { ...i, x: clamp(i.x + dx, 2, 98), y: clamp(i.y + dy, 2, 98) } : i,
          ),
        })
      }
    }
  ...
  // Hauteur (en % de la carte) d'une image posée, d'après sa largeur et son ratio.
  const imgHpct = (im: CardImage) => im.size * (im.aspect || 1) * ASPECT
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_18rem]">
      ...
      >
        ...
        {/* Hotspots des IMAGES importées posées */}
        {(card.images ?? []).map((im) => (
          <Hotspot
            key={im.id}
            left={im.x - im.size / 2}
            top={im.y - imgHpct(im) / 2}
            width={im.size}
            height={imgHpct(im)}
            selected={sel?.kind === 'image' && sel.id === im.id}
            onDown={(e) => startDrag(e, { kind: 'image', id: im.id }, 'move')}
            onResize={(e) => startDrag(e, { kind: 'image', id: im.id }, 'resize')}
            onDelete={() => removeImage(im.id)}
          />
        ))}
      </div>
      {belowPreview}
      ...
      {/* Barre : importer une image à poser librement sur la face. */}
      <div className="flex flex-col gap-1">
        <input
          ref={imgFileRef}
          type="file"
        ...
        <button
          ...
          className="self-start rounded-lg border border-emerald-400/40 bg-emerald-400/10 px-2 py-1 text-xs font-semibold text-emerald-100 transition hover:bg-emerald-400/20"
        >
          🖼 Ajouter une image
        </button>
        {sel?.kind === 'image' &&
          (() => {
            const im = card.images?.find((i) => i.id === sel.id)
            if (!im) return null
            const sizeBtn = (label: string, size: number) => (
              <button
                key={label}
                type="button"
                onClick={() => setImage(im.id, { size })}
                className={`rounded-lg border px-2 py-1 text-xs font-semibold transition ${
                  im.size === size
                    ? 'border-amber-400 bg-amber-400/20 text-amber-100'
                    : 'border-white/20 bg-white/5 text-white/80 hover:border-amber-300/70 hover:text-amber-200'
            ...
            return (
              <div className="mt-1 flex flex-col gap-1.5 rounded-lg border border-emerald-400/20 bg-emerald-400/5 px-2 py-1.5">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-[11px] font-semibold uppercase tracking-wide text-emerald-200/70">Image sélectionnée</span>
                  <div className="flex items-center gap-1.5">
                    <button
                      type="button"
                      onClick={() => dupImage(sel.id)}
                ...
                {/* Centrer horizontalement l'image (x = 50 %). */}
                <div className="flex flex-wrap items-center gap-1.5">
                  <button
                    type="button"
                    onClick={() => setImage(im.id, { x: 50 })}
                    title="Centrer l’image horizontalement au milieu de la carte"
                    className="rounded-lg border border-white/20 bg-white/5 px-2 py-1 text-xs font-semibold text-white/80 transition hover:border-amber-300/70 hover:text-amber-200"
                  >
                    ⇔ Centrer H
                  </button>
                </div>
                {/* Largeur de l'image (déplaçable/redimensionnable au drag aussi). */}
                <div className="flex flex-wrap items-center gap-1.5">
                  <span className="text-[11px] text-white/40">Taille :</span>
                  {sizeBtn('Petite', 20)}
                  {sizeBtn('Moyenne', 30)}

...
export function FateReorderModal({ cards, onResolve, title = 'Je ne reviens jamais sur ma parole', deckLabel = 'Fatalité' }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/75 p-4">
      <div className="flex w-full max-w-2xl flex-col gap-4 rounded-2xl border border-white/15 bg-[#15101f] p-6 text-white shadow-2xl">
          ...
          {cards.map((c) => {
            ...
            return (
              ...
              >
                ...
                <span className="mt-0.5 block text-center text-[10px] text-white/70">{c.name}</span>
                {picked && (

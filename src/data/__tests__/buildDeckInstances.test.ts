    ...
    v.cards = [
      {
        ...
        images: [{ id: 'im1', image: 'data:image/png;base64,BBBB', x: 20, y: 20, size: 30, aspect: 1 }],
      },
    ]
    const [inst] = buildDeckInstances(toDeckCardDefs(v), 'villain')
    const rec = inst as unknown as Record<string, unknown>
    expect(inst.strength).toBe(4) // le champ de jeu passe
    for (const k of ['artImage', 'artTransform', 'typeLabel', 'typeColor', 'textLayout', 'stickers', 'images', 'group']) {
